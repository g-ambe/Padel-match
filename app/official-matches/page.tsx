...
type OfficialEvent = { id: string; club_id: string; title: string; event_date: string | null; status: string; share_enabled?: boolean | null; share_token?: string | null; clubs: { name: string } | null };
...
export default function OfficialMatchListPage() {
  ...
  useEffect(() => {
    void (async () => {
      ...
      let query = supabase.from("official_events").select("id,club_id,title,event_date,status,share_enabled,share_token,clubs(name)").in("club_id", clubIds);
