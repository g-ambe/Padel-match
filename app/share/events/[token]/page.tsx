...
type Row = { name: string; played: number; wins: number; decided: number; scored: number; conceded: number; winRate: number; diff: number };
...
export default function SharedEventPage() {
  ...
  const rows = useMemo(() => {
    ...
    for (const p of participants) t[p.id] = { name: p.display_name, played: 0, wins: 0, scored: 0, conceded: 0, decided: 0, winRate: 0, diff: 0 };
    for (const m of matches) {
      ...
      for (const pid of teamA) { const r = t[pid]; if (!r) continue; r.played++; r.scored += m.result.score_a; r.conceded += m.result.score_b; if (m.result.winner_team !== "draw") r.decided++; if (m.result.winner_team === "A") r.wins++; }
      for (const pid of teamB) { const r = t[pid]; if (!r) continue; r.played++; r.scored += m.result.score_b; r.conceded += m.result.score_a; if (m.result.winner_team !== "draw") r.decided++; if (m.result.winner_team === "B") r.wins++; }
    }
    return Object.values(t).map((r) => ({ ...r, winRate: r.decided ? Math.round((r.wins / r.decided) * 1000) / 10 : 0, diff: r.scored - r.conceded }));
